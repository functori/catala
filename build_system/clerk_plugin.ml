...
type t = (module Clerk_backends.Backend.S)

let clerk_plugins : (Clerk_config.backend, t) Hashtbl.t = Hashtbl.create 17

let register name cons (module B : Clerk_backends.Backend.S) =
  Clerk_config.register_backend ~name cons;
  Hashtbl.replace clerk_plugins cons (module B)

let list () = Hashtbl.to_seq_values clerk_plugins |> List.of_seq
let names () = Hashtbl.to_seq_keys clerk_plugins |> List.of_seq
  ...
  Array.iter
    (fun f ->
      ...
      if
        ...
        && List.exists (Filename.check_suffix f) dynlink_exts
      then load_file f)
    (Sys.readdir d)
