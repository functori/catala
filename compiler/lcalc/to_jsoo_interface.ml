...
let format_ctx
    ...
    if TypeIdent.(Set.mem (Struct struct_name) ctx.ctx_public_types) then
      ...
      if StructField.Map.is_empty struct_fields then (
        ...
        let () =
          if include_ then
            Format.fprintf ppml "include %a@," To_ocaml.format_to_module_name
              (`Sname struct_name)
          else Format.fprintf ppml "type t = unit@,"
        in
        Format.fprintf ppml
          "type jsoo = unit@,\
           let to_jsoo = Fun.id@,\
        ...
        (let type_printer =
         ...
         in
         Format.fprintf ppi "@[<v 2>module %a : sig@,@[<hv 2>type "
           To_ocaml.format_to_module_name (`Sname struct_name);
         let () =
           if include_ then
  ...
  let format_enum_decl (enum_name, enum_cons) =
    if TypeIdent.(Set.mem (Enum enum_name) ctx.ctx_public_types) then
      ...
      in
      match content_cons_variants with
      | [] ->
        (* If there is no variant with parameter, the conversion for all variant
           of the enum is to a string *)
        ...
        Format.fprintf ppi
          "%a%a%a@]@,\
           ...
           <1 -2>end@]@,\
           @,"
          Format.pp_print_if_newline () Format.pp_print_string "| " enum_printer
          variants;
        ...
        Format.fprintf ppml
          ...
          variants
      | _ -> (
        (* If there are variant with parameter, the interface is unchanged but
        ...
        Format.fprintf ppi
          ...
          variants;
        match empty_cons_variants with
        | [] ->
          (* If there are no empty variant, every javascript representation of a
             case of the enum is an object with a field. So A of int is
             represented as {"A": 0} for example *)
          Format.fprintf ppml "@[<hv 2>module %a = struct@ @[<hv 2>"
            To_ocaml.format_to_module_name (`Ename enum_name);
          print_type_ppml ();
          Format.fprintf ppml
            ...
            (Format.pp_print_list
               ...
               (fun fmt (enum_cons, enum_cons_type) ->
                 Format.fprintf fmt
                   "@[<hov 2>%a x -> object%%js@;<1 0>%a@;<1 -2>end@]@,"
                   To_ocaml.format_enum_cons_name enum_cons
                   (Format.pp_print_list
                      ~pp_sep:(fun fmt () -> Format.fprintf fmt "@,")
                      (fun fmt (enum_cons2, _) ->
                        if enum_cons <> enum_cons2 then
                          ...
                          Format.fprintf fmt
                            "@[<hov 2>val mutable %a =@ Js.def (%a@ x)@]@,"
            ...
            variants
        | _ ->
          (* Unfortunately if the enum is a mix of empty variant and
             parameterized variant, we must check for the javascript type of the
             js object, if it's a string we should look on the side of the empty
             variant, if it's not a string look at the side of the parameterized
             variant *)
          Format.fprintf ppml "@[<hv 2>module %a = struct@ @[<hv 2>"
            To_ocaml.format_to_module_name (`Ename enum_name);
          print_type_ppml ();
            ...
            (Format.pp_print_list
               ...
               (fun fmt (enum_cons, enum_cons_type) ->
                 Format.fprintf fmt "@[<hov 2>%a -> %a@ (%a _c)@]@,"
                   ...
                   content_cons_variants To_ocaml.format_enum_cons_name
                   enum_cons format_typ_of enum_cons_type))
            content_cons_variants)
  in
