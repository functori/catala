...
let runtime_ocaml backend ~ocaml_src ~dates_base ~ocaml_base =
  ...
  [
    ...
    Nj.build "copy" ~inputs:[dates_cmi -.- "ml"] ~outputs:[dates_base -.- "ml"];
    Nj.build "copy"
...
module Backend = struct
  ...
  let module_ext = "@" ^ backend_name ^ "-module"
  let subdir = backend_name
  ...
  let external_copy item =
    ...
    let ml, missing =
      Ninja.extern_src ~filename:item.Scan.file_name ~backend:backend_name
        ~ext:"ml" ~missing:[]
    in
    let mli, missing =
      Ninja.extern_src ~filename:item.Scan.file_name ~backend:backend_name
        ~ext:"mli" ~missing
    in
    Ninja.check_missing ~backend:backend_name ~module_def:item.Scan.module_def
      ~missing ~filename:item.file_name;
    List.to_seq
      [
        Nj.build "copy" ~implicit_in:[catala_src] ~inputs:[ml]
          ~outputs:[Ninja.target ~backend:backend_name "ml"];
        Nj.build "copy" ~implicit_in:[catala_src] ~inputs:[mli]
          ~outputs:[Ninja.target ~backend:backend_name "mli"];
      ]
